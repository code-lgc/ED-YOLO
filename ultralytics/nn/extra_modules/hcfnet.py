...
class SparseDASI(nn.Module):
    def __init__(self, c, n_scales=3, tau=0.3):
        super().__init__()
        self.tau = tau
        self.weight = nn.Sequential(
            nn.AdaptiveAvgPool2d(1),
            nn.Conv2d(c, n_scales, 1)
        )

    def forward(self, feats):  # feats: list[P3, P4, P5]
        w = torch.softmax(self.weight(sum(feats)), dim=1)  # [B,3,1,1]
        mask = (w > self.tau).float()
        w = w * mask
