...
#######################################     Gaussian Blur  +   GlobalEdgeInformationTransfer  

# class DepthwiseFixedConv2d(nn.Module):
#     def __init__(self, channel, kernel, padding=1):
#         super().__init__()
#         k = torch.tensor(kernel, dtype=torch.float32)
#         k = k.unsqueeze(0).unsqueeze(0)                  # (1,1,kh,kw)
#         k = k.expand(channel, 1, *k.shape[-2:]).contiguous()  # (C,1,kh,kw)
#         self.conv = nn.Conv2d(channel, channel, k.shape[-1], padding=padding, groups=channel, bias=False)
...
# class BlurScharrEdge(nn.Module):
#     def __init__(self, channel, blur_ks=3):
#         super().__init__()
#         # 简单高斯核(3x3)，你也可以换 5x5
#         if blur_ks == 3:
#             g = np.array([[1,2,1],
...
#         self.blur = DepthwiseFixedConv2d(channel, g, padding=pad)
#         self.gx   = DepthwiseFixedConv2d(channel, scharr_x, padding=1)
#         self.gy   = DepthwiseFixedConv2d(channel, scharr_y, padding=1)

#     def forward(self, x):
#         x = self.blur(x)
#         grad = 0.5 * self.gx(x) + 0.5 * self.gy(x)  # 你也可以用 abs/gmag
#         return grad
